...
- **Model/embeddings**: `open_clip` ViT-B/32 (`openai` weights). Image/text similarities use CLIP logits (`logit_scale * cosine`). Images are preprocessed with the model defaults; text is tokenized with `open_clip.tokenize`.
...
- `clip_new_prompt_sim` = CLIP logit similarity between the edited image embedding `E_edit` and the target prompt embedding `T_new`:\
  `logit_scale * cos(E_edit, T_new)` where `logit_scale = exp(model.logit_scale)`
- `clip_old_image_sim` = CLIP logit similarity between the edited image embedding `E_edit` and the old image embedding `E_old` (`idx=0` of the same type):\
  `logit_scale * cos(E_edit, E_old)`
